from Config.config import user_token, comm_token, line
import vk_api
import requests
import datetime
...
from Object.keyboard import get_button, keyboard
    ...
    def start(self):
        ...
        for event in self.longpoll.listen():
            if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                ...
                self.sender(user_id, msg.lower())
                if request == "начать поиск":
                    creating_database()
                    self.write_msg(user_id, f"Привет, {self.name(user_id)}")
                    self.find_user(user_id)
                    self.write_msg(
                        event.user_id, f'Нашёл для тебя пару, жми на кнопку "Вперёд"'
                    )
                    self.find_persons(user_id, offset)
                    ...
                    self.write_msg(event.user_id, "Твоё сообщение непонятно")
